 ...
 */
export const getIdFromProduct = (product) => product.querySelector('span.product__id').innerText;
...
export const createCartProductElement = ({ id, title, price, pictures }) => {
  ...
  priceElement.appendChild(
    createCustomElement('span', 'product__price__value', price),
  );
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', () => {
    ...
    const addProductToCart = async () => {
      ...
      const total = convertToNumber + productElement.price;
      totalPrice.innerText = total.toFixed(2);
